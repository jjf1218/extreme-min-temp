"""
Bin Separation Scripts - Edited for Percents
...
"""
# A list of the precip categories
precip_category = ["A", "B", "C", "D", "E", "F", "G", \
                   "H", "I", "J", "K", "L", "M", "N"]

# We initialize the dictionary to sort the number of occurances of a category
precip_freq = {}
for yr in range(int(date[0][0:4]),int(date[-1][0:4])+1):
    precip_freq[yr] = {key: 0 for key in precip_category}
    
# We initlialize a dictionary to add in actual values
precip_amount = {}
for yr in range(int(date[0][0:4]),int(date[-1][0:4])+1):
    precip_amount[yr] = {key: 0 for key in precip_category}

# Using this method, we will add a count of the number of category events into
# precip_freq, and add the actual count into precip_amount
for i in range(len(date)):
    if precip_dict[int(date[i][0:4])][date[i]] < 0.03:
        ...
        precip_amount[int(date[i][0:4])]["B"] += precip_dict[int(date[i][0:4])][date[i]]
    elif precip_dict[int(date[i][0:4])][date[i]]  < 0.26:
        precip_freq[int(date[i][0:4])]["C"] += 1
        precip_amount[int(date[i][0:4])]["C"] += precip_dict[int(date[i][0:4])][date[i]]
    elif precip_dict[int(date[i][0:4])][date[i]]  < 0.51:
        precip_freq[int(date[i][0:4])]["D"] += 1
        precip_amount[int(date[i][0:4])]["D"] += precip_dict[int(date[i][0:4])][date[i]]
    elif precip_dict[int(date[i][0:4])][date[i]]  < 1.01:
        precip_freq[int(date[i][0:4])]["E"] += 1
        precip_amount[int(date[i][0:4])]["E"] += precip_dict[int(date[i][0:4])][date[i]]
    elif precip_dict[int(date[i][0:4])][date[i]]  < 1.51:
        precip_freq[int(date[i][0:4])]["F"] += 1
        precip_amount[int(date[i][0:4])]["F"] += precip_dict[int(date[i][0:4])][date[i]]
    elif precip_dict[int(date[i][0:4])][date[i]]  < 2.01:
        precip_freq[int(date[i][0:4])]["G"] += 1
        precip_amount[int(date[i][0:4])]["G"] += precip_dict[int(date[i][0:4])][date[i]]
    elif precip_dict[int(date[i][0:4])][date[i]]  < 2.51:
        precip_freq[int(date[i][0:4])]["H"] += 1
        precip_amount[int(date[i][0:4])]["H"] += precip_dict[int(date[i][0:4])][date[i]]
    elif precip_dict[int(date[i][0:4])][date[i]]  < 3.01:
        precip_freq[int(date[i][0:4])]["I"] += 1
        precip_amount[int(date[i][0:4])]["I"] += precip_dict[int(date[i][0:4])][date[i]]
    elif precip_dict[int(date[i][0:4])][date[i]]  < 4.01:
        precip_freq[int(date[i][0:4])]["J"] += 1
        precip_amount[int(date[i][0:4])]["J"] += precip_dict[int(date[i][0:4])][date[i]]
    elif precip_dict[int(date[i][0:4])][date[i]]  < 5.01:
        precip_freq[int(date[i][0:4])]["K"] += 1
        precip_amount[int(date[i][0:4])]["K"] += precip_dict[int(date[i][0:4])][date[i]]
    elif precip_dict[int(date[i][0:4])][date[i]]  < 6.01:
        precip_freq[int(date[i][0:4])]["L"] += 1
        precip_amount[int(date[i][0:4])]["L"] += precip_dict[int(date[i][0:4])][date[i]]
    elif precip_dict[int(date[i][0:4])][date[i]]  < 7.01:
        precip_freq[int(date[i][0:4])]["M"] += 1
        precip_amount[int(date[i][0:4])]["M"] += precip_dict[int(date[i][0:4])][date[i]]
    else:
        precip_freq[int(date[i][0:4])]["N"] += 1
        precip_amount[int(date[i][0:4])]["N"] += precip_dict[int(date[i][0:4])][date[i]]

# Now we want to make sure the floats round to 2 decimal places

"""
for k, v in precip_amount.iteritems():
    for value in precip_amount[k][v]:
        precip_amount[k][v]
"""
